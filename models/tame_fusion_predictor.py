"""TAME-Fusion predictor: SEG fusion (Graph + Text) with descriptor-aware 2-gate element-wise MoE."""
...
from models.core.fusion import BaseFusion, create_fusion
...
class ModalityProjectionMLP(nn.Module):
    """Project an embedding to a shared hidden dimension."""
...
class SEGDescriptorElementWiseMoE(nn.Module):
    """
    Two-branch fusion module with element-wise mixture-of-experts routing.
    ...
    Steps:
    1) Project each branch to hidden_dim
    2) Route with element-wise softmax gates over 2 experts (SEG, Descriptor)
    3) Fuse as weighted element-wise sum
    4) Predict task outputs
    """
    ...
    def forward(
        ...
        e_s = self.seg_proj(seg_emb)
        e_d = self.desc_proj(desc_features)

        concat = torch.cat([e_s, e_d], dim=-1)
        ...
        fused_emb = (e_s * g_s) + (e_d * g_d)
        pred = self.pred_head(fused_emb)

        aux = {
            "seg_gate": g_s,
            "desc_gate": g_d,
            "gates_full": gates,  # (B, H, 2) — used for entropy regularisation
            "fused_emb": fused_emb,
        }
        return pred, aux

    def _init_router_at_target(self, gate_target: float) -> None:
        import math
        final = self.router[-1]
...
@dataclass
class TAMEFusionPredictorConfig(BasePredictorConfig):
    """Configuration for TAMEFusionPredictor."""
    ...
    # SEG-style graph/text fusion
    fusion: str = "cross_mha"
    fusion_dim: int = 256
    fusion_n_heads: int = 8
    fusion_dropout: float = 0.1

    # 2-gate MoE: SEG branch vs Descriptor branch
    ...
    # Target gate value for the SEG branch (desc branch target = 1 - gate_target)
...
class TAMEFusionPredictor(BasePredictor):
    """Predictor using SEG fusion + descriptor-aware 2-gate element-wise MoE."""

    def __init__(
        self,
        config: Optional[TAMEFusionPredictorConfig] = None,
        text_embedding_fn: Optional[Callable[[List[str]], np.ndarray]] = None,
        descriptor_fn: Optional[Callable[[List[str]], np.ndarray]] = None,
        device: Optional[str] = None,
    ) -> None:
        self.config = config or TAMEFusionPredictorConfig()
        ...
        self._text_proj: Optional[nn.Module] = None
        self._fusion: Optional[BaseFusion] = None
        self._moe_module: Optional[SEGDescriptorElementWiseMoE] = None
    ...
    def fit(
        ...
        *,
        ...
        other_params = (
            (list(self._pooling.parameters()) if self._pooling is not None else [])
            + (list(self._text_proj.parameters()) if self._text_proj is not None else [])
            + (list(self._fusion.parameters()) if self._fusion is not None else [])
            + (list(self._moe_module.parameters()) if self._moe_module is not None else [])
        )
        ...
        history = {"train_loss": [], "val_loss": [], "val_auc": [], "seg_gate": [], "desc_gate": []}
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            history["seg_gate"].append(float(gate_stats[0]))
            history["desc_gate"].append(float(gate_stats[1]))
            ...
            if verbose and (epoch == 1 or epoch % 5 == 0):
                ...
                print(
                    ...
                    f"gates SEG/D={gate_stats[0]:.3f}/{gate_stats[1]:.3f}"
                )
        ...
        torch.save(
            {
                ...
                "text_proj_state": self._text_proj.state_dict() if self._text_proj is not None else None,
                "fusion_state": self._fusion.state_dict() if self._fusion is not None else None,
    ...
    def load(self, path: str) -> None:
        ...
        self.config = TAMEFusionPredictorConfig(**ckpt["config"])
        ...
        if self._encoder is None or self._pooling is None or self._fusion is None or self._moe_module is None:
        ...
        if self._text_proj is not None and ckpt.get("text_proj_state", None) is not None:
    ...
    def _build_model(self, desc_dim: int) -> None:
        ...
        text_dim_for_fusion = int(self.config.text_embedding_dim)
        text_proj_dim = self.config.text_projection_dim
        if text_proj_dim is not None and int(text_proj_dim) < int(self.config.text_embedding_dim):
            self._text_proj = nn.Sequential(
                nn.Linear(int(self.config.text_embedding_dim), int(text_proj_dim)),
                nn.LayerNorm(int(text_proj_dim)),
                nn.GELU(),
                nn.Dropout(float(self.config.fusion_dropout)),
            ).to(self.device)
            text_dim_for_fusion = int(text_proj_dim)
        else:
            self._text_proj = None

        graph_dim = int(self._pooling.output_dim)
        fusion_kwargs = {
            "output_dim": int(self.config.fusion_dim),
            "dropout": float(self.config.fusion_dropout),
        }
        if self.config.fusion == "cross_mha":
            fusion_kwargs["n_heads"] = int(self.config.fusion_n_heads)
        ...
        self._moe_module = SEGDescriptorElementWiseMoE(
            seg_dim=int(self._fusion.output_dim),
            desc_dim=int(desc_dim),
            num_tasks=int(self.config.num_tasks),
            hidden_dim=int(self.config.moe_hidden_dim),
    ...
    def _forward(
        ...
        if (
            ...
            or self._pooling is None
            or self._fusion is None
            or self._moe_module is None
        ):
        ...
        seg_emb = self._fusion(graph_emb, text_t)
        return self._moe_module(seg_emb, desc_t)
    ...
    def _train_epoch(
        ...
        gate_target = float(self.config.gate_target)
        ...
        for start in range(0, len(order), batch_size):
            ...
            # so SEG branch remains useful during training.
            if desc_drop_p > 0.0:
            ...
            gate_sums[0] += float(aux["seg_gate"].mean().detach().cpu().item())
            gate_sums[1] += float(aux["desc_gate"].mean().detach().cpu().item())
    ...
    def _evaluate(
        ...
        res: Dict[str, float] = {"val_loss": total_loss / max(n_total, 1)}
    ...
    def _get_state_dict(self) -> Dict[str, Dict[str, torch.Tensor]]:
        ...
        if self._text_proj is not None:
            state["text_proj"] = self._text_proj.state_dict()
        return state
