...
# Füge Projektwurzel zum Python-Pfad hinzu
...
def find_best_threshold(y_true, y_proba):
    """Sucht den besten F1-Schwellenwert zwischen 0.05 und 0.95."""
...
def compute_rdkit_descriptors(smiles_list):
    """Berechnet RDKit Deskriptoren (exakt wie im Notebook)."""
...
def impute_with_train_median(train_X, valid_X, test_X):
    """Führt ein einfaches NaN-Imputing mit dem Train-Median durch (wie im Notebook)."""
...
# Globaler Cache, damit Daten nicht für jedes HPO Trial neu geladen werden müssen
GLOBAL_DATA_CACHE = {}


def get_data(splitter):
    """Lädt den BACE Datensatz und Modalitäten exakt wie im Ablations-Notebook in den RAM."""
    if splitter in GLOBAL_DATA_CACHE:
        return GLOBAL_DATA_CACHE[splitter]

    print(
        f"\n[Info] Initiales Laden der BACE-Daten ({splitter.title()} Split) in RAM..."
    )
    ...
    print(f"[Info] Lade Text-Embeddings von {emb_path}...")
    ...
    # RDKit Deskriptoren
    print("[Info] Berechne RDKit Deskriptoren und imputiere fehlende Werte...")
...
def objective(trial, args):
    ...
    # Optuna Suchraum - ChebNet Backbone Architektur
    ...
    gate_target_graph = trial.suggest_float("gate_target_graph", 0.3, 0.8)
    gate_target_text = trial.suggest_float(
        "gate_target_text", 0.0, 1.0 - gate_target_graph
    )
    gate_target_desc = 1.0 - gate_target_graph - gate_target_text
    ...
    try:
        ...
        if not os.path.exists(checkpoint_path):
            print(f"[Info] Überspringe Trial: Checkpoint {ckpt_name} existiert nicht.")
        ...
        config = TAMEPredictorConfig(
            ...
            descriptor_dim=int(data["desc_train"].shape[1]),
            fusion_hidden_dim=proj_dim,
            ...
            gate_target=(gate_target_graph, gate_target_text, gate_target_desc),
        ...
        model = TAMEPredictor(config=config, device=device)

        # Encoder Status extrahieren
        ...
        # Aufruf exakt wie im Jupyter-Notebook!
        _ = model.fit(
            ...
            patience=20,  # Notebook Early-Stopping Metrik auf BCE Validation Loss
            verbose=False,
        ...
        if len(y_val_obs) == 0:
            raise ValueError("Keine gültigen Predictions im Validation Set.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Optuna HPO for TAMEPredictor using built-in `.fit()`"
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        default="Rechnungen/hpo",
        help="Verzeichnis für HPO Ergebnisse",
    )
    parser.add_argument(
        "--ckpt_dir",
        type=str,
        required=True,
        help="Ordner mit den vortrainierten ChebNet Checkpoints",
    )
    parser.add_argument("--seed", type=int, default=11, help="Seed für HPO")
    parser.add_argument(
        "--splitter",
        type=str,
        choices=["random", "scaffold"],
        default="scaffold",
        help="DeepChem Splitter",
    )
    parser.add_argument("--n_trials", type=int, default=50, help="Anzahl Optuna Trials")
    parser.add_argument(
        "--max_epochs",
        type=int,
        default=120,
        help="Max Training Epochen (Notebook default ist 120)",
    )
    parser.add_argument(
        "--metric",
        type=str,
    ...
    parser.add_argument(
        ...
        default="val_loss",
        help="Metrik für das Early Stopping innerhalb von fit()",
    )
    ...
    db_url = f"sqlite:///{
        os.path.abspath(
            os.path.join(
                args.out_dir, f'tame_predictor_hpo_v3_{args.splitter}_{seed_tag}.db'
            )
        ).replace(chr(92), '/')
    }"

    study = optuna.create_study(
        study_name=f"tame_predictor_hpo_v3_{args.splitter}_{seed_tag}",
        direction="maximize",
        storage=db_url,
        load_if_exists=True,
        sampler=optuna.samplers.TPESampler(seed=args.seed),  # Kein Pruner mehr nötig!
    )

    print(f"Starte Optuna HPO Studie v3 (Trials: {args.n_trials})...")
    study.optimize(lambda trial: objective(trial, args), n_trials=args.n_trials)

    print("\nStudie beendet! Beste Parameter:")
    for k, v in study.best_trial.params.items():
        print(f"  {k}: {v}")

    out_path = os.path.join(
        args.out_dir, f"tame_predictor_best_params_v3_{args.splitter}_{seed_tag}.json"
    )
